...
# from app.config.config_manager import ConfigManager  # 避免循环导入
...
class ConfigWatcher:
    ...
    def _on_global_config_changed(self, file_path: str):
        ...
        try:
            ...
            coro = self.config_manager._load_global_config()
            ...
            try:
                ...
                if loop.is_running():
                    loop.call_soon_threadsafe(
                        lambda: asyncio.create_task(self.config_manager._load_connections_config())
                    )
                else:
                    # 明确用 run_until_complete 并关闭循环
                    loop.run_until_complete(self.config_manager._load_connections_config())
                    loop.close()
            except RuntimeError:
                # 如果当前线程没有 loop，创建新的
                loop = asyncio.new_event_loop()
    ...
    def _on_account_config_changed(self, file_path: str):
        ...
        try:
            ...
            coro = self.config_manager._load_account_configs()

            try:
                loop = asyncio.get_event_loop()
                if loop.is_running():
                    # 当前在运行的事件循环中，安全地创建新任务
                    loop.call_soon_threadsafe(lambda: asyncio.create_task(coro))
                else:
                    # 有 loop 但未运行，直接运行该协程
                    loop.run_until_complete(coro)
    ...
    def _on_group_config_changed(self, file_path: str):
        ...
        try:
            ...
            coro = self.config_manager._load_group_configs()

            try:
                loop = asyncio.get_event_loop()
                if loop.is_running():
                    # 当前 loop 正在运行，线程安全地创建协程任务
                    loop.call_soon_threadsafe(lambda: asyncio.create_task(coro))
                else:
                    # loop 存在但没运行，直接运行协程
                    loop.run_until_complete(coro)
