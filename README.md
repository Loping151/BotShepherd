...
### 2026-06-01 v1.1.4 重连退避优化 + 修复月末备份

- 目标重连：固定每 10 分钟的长尾改为「4s 起每次 +1s、封顶 300s」的渐进退避，较长下线恢复更快、长尾封顶（短抖动仍走 3s 快重连）
- 修复每日备份月末报错（`day is out of range for month`）：用 timedelta 计算次日，正确跨月末/年末
