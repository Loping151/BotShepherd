...
### 2026-07-19 v1.2.1 安全更新

- 安全性更新，建议所有用户尽快升级到最新版本。

<details>
<summary>点此展开历史版本</summary>

2026-07-16 v1.2.0 内置发言榜插件

- 新增 `bs发言榜 [日期] [刷新]`（别名 水群榜），群管理员以上可用，统计本群发言排行出图；日期支持 昨天/N天前/本周/上月/7月5日 等，周期以每日 08:00 为界，依赖新增 pillow
- `app/plugins/` 加入 gitignore，用户可在此编写自己的插件不污染仓库（内置插件保持跟踪，新增内置需 `git add -f`）

2026-06-01 v1.1.4 重连退避优化（固定 10 分钟改 4s 起步封顶 300s 渐进退避）+ 修复每日备份月末跨月报错

2026-05-30 v1.1.3 过滤词支持 ^/$ 锚点：新增 `^` 开头、`$` 结尾锚点（非完整正则），可与 +/| 组合；群过滤改多候选独立匹配；毫秒时间戳归一化
...
2026-01-10 v1.0.1 小修，修复显示、账号在线检查
...
仅支持单次回复，支持图文。`app/plugins/` 已加入 gitignore，可直接在其中编写自己的插件（单文件或包目录，启动时自动加载），示例见内置插件：`bs_plugin_ping.py` 最小示例、`bs_plugin_talkrank` 查库+PIL 出图完整示例。
