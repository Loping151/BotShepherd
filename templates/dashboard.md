...
- **全局过滤词** (`global_filters`)
  - **接收过滤** (`receive_filters`)：阻止包含特定词汇的消息传递给框架。群管设置的群过滤可以用于**过滤不想要的功能**，比如今日运势，则机器人不再收到今日运势请求。仅限群过滤，还会匹配发送者与机器人。可以把ID作为群过滤词来在群里**忽略某个人**或**禁用某个机器人**。为了进一步实现某群某机禁用某功能，所有过滤词支持 and 和 or 模式，分别使用 `+` 和 `|` 连接。例如：某群的bot12345不想要收到abc，可以设置过滤词为 `abc+12345`。
    - 举例：`["广告", "刷单", "代练"]`
  - **发送过滤** (`send_filters`)：阻止包含特定词汇的消息发送给客户端，主要针对ai回复等**不可控内容**。同样可以使用 and 和 or 模式。
    - 举例：`["敏感词", "违禁内容"]`
