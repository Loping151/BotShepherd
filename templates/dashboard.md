...
- **触发前缀❗** (`trigger_prefix`)
  ...
  - 说明：用于模拟其他用户发送指令，帮助其他用户执行指令。该配置只能在配置文件中修改。**Bot自身也可以触发。**
...
- **全局过滤词** (`global_filters`)
  - **接收过滤** (`receive_filters`)：阻止包含特定词汇的消息传递给框架。群管设置的群过滤可以用于**过滤不想要的功能**，比如今日运势，则机器人不再收到今日运势请求。仅限群过滤，还会匹配发送者与机器人。可以把QQ号作为群过滤词来在群里**忽略某个人**或**禁用某个机器人**。
    - 举例：`["广告", "刷单", "代练"]`
  - **发送过滤** (`send_filters`)：阻止包含特定词汇的消息发送给客户端，主要针对ai回复等**不可控内容**
    - 举例：`["敏感词", "违禁内容"]`
...
- **目标端点列表** (`target_endpoints`)
  ...
  - 举例：`["ws://localhost:3001/OneBot/v11/ws", "ws://localhost:3002/ws"]`
  - 说明：支持一对多转发。注意原先客户端填写的目标是什么，这里就写什么，需要包含路径
